...
output:
  - name: "모델 체크포인트 파일"
    desc: "학습된 정책의 가중치(weight) 파일이 저장됩니다. 정책 종류(ACT, Diffusion, pi0, pi05, SmolVLA, GR00T N1 등)에 따라 파일 형식이 다르지만, 모두 지정한 출력 폴더 안에 들어갑니다. Inference 화면에서 이 폴더를 선택하면 로봇이 학습한 대로 움직입니다"
    path: "{output_folder_name}/checkpoints/"
  - name: "학습 설정 파일 (config.json)"
    desc: "학습에 사용한 모든 설정(정책 종류, 데이터셋 경로, 하이퍼파라미터, 배치 사이즈, 학습률 등)이 JSON으로 저장됩니다. 나중에 같은 설정으로 재학습하거나, Inference에서 정책을 로딩할 때 이 파일을 참조합니다"
    path: "{output_folder_name}/config.json"
  - name: "Loss 그래프 (화면 표시)"
    desc: "학습 중 loss 값이 실시간 그래프로 표시됩니다. loss가 점점 내려가면 학습이 잘 되고 있다는 뜻입니다. 그래프가 평평해지거나 올라가면 학습을 중단하고 데이터나 설정을 점검해야 합니다"
  - name: "학습 히스토리"
    desc: "[btn:History]에서 이전 학습 실험들의 기록(데이터셋, 정책, loss, 학습 시간)을 확인할 수 있습니다"
