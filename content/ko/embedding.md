...
output:
  - name: "Embedding 산점도 (화면 표시만)"
    desc: "파일로 저장되는 건 없습니다. 모델이 데이터를 내부적으로 어떻게 구분하는지 2D 지도(t-SNE, PCA, UMAP)로 보여줍니다. 비슷한 데이터가 뭉쳐 있으면 모델이 패턴을 잘 학습했다는 신호이고, 뒤섞여 있으면 학습이 더 필요하거나 데이터에 문제가 있을 수 있습니다"
  - name: "비교 분석 결과"
    desc: "여러 데이터셋을 추가하면 데이터셋별/에피소드별/작업별로 색이 다르게 표시되어 모델이 어떤 차이를 구분하고 있는지 한눈에 볼 수 있습니다"
