...
output:
  - name: "tf_offset 보정값"
    desc: "카메라 좌표계와 로봇 좌표계 사이의 변환 행렬(translation XYZ + rotation quaternion)이 계산됩니다. 이 값을 로봇 설정 YAML 파일에 넣으면 카메라가 본 물체 위치를 로봇이 정확히 이해할 수 있습니다"
  - name: "RMSE 오차값"
    desc: "캘리브레이션 정확도를 나타내는 RMSE(Root Mean Square Error) 수치가 표시됩니다. 값이 작을수록 정확합니다. 다양한 자세로 6~10회 이상 점을 찍어야 실용적인 RMSE가 나옵니다"
  - name: "체크리스트 (복사용)"
    desc: "[btn:Copy Checklist]로 보정값과 적용 순서를 텍스트로 복사할 수 있습니다. YAML 설정에 붙여넣기 편하게 포맷되어 있습니다"
