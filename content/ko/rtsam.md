...
output:
  - name: "실시간 추적 상태 (화면 표시만)"
    desc: "파일로 저장되는 건 없습니다. 라이브 카메라 영상 위에 물체 마스크가 실시간으로 표시됩니다. 추적 성공/거부 카운터, 지연 시간(latency), 모델 상태가 하단에 표시되어 추적 품질을 실시간으로 판단할 수 있습니다"
  - name: "마스크 안정성 확인"
    desc: "물체가 움직이거나 카메라가 이동해도 마스크가 물체를 잘 따라가는지 확인합니다. 표류(drift)가 발생하면 추적을 멈추고 물체를 다시 잠금해야 합니다. RAIN 화면에서 활용하기 전 여기서 미리 테스트하는 것을 권장합니다"
