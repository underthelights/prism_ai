...
output:
  - name: "EEF 포즈 컬럼 (observation.eef_pose)"
    desc: "기존 parquet 파일에 새 컬럼이 추가됩니다. 각 프레임마다 7개 값: XYZ 위치(미터) + 쿼터니언 회전(qx, qy, qz, qw). UR5 기준 Base Link=link0, Target Link=end_effector_link로 변환합니다. 이 값이 있어야 Visualize의 3D 궤적, Subtask의 자동 분할이 작동합니다"
    path: "data/chunk-000/episode_XXXXXX.parquet → observation.eef_pose 컬럼 추가"
  - name: "EEF 궤적 미리보기 (화면 표시)"
    desc: "변환 후 XY, XZ 평면의 2D 궤적이 화면에 표시됩니다. 궤적이 자연스럽게 이어지면 변환 성공, 끊기거나 튀는 구간이 있으면 프레임 이름이나 데이터를 재확인해야 합니다"
  - name: "info.json 업데이트"
    desc: "meta/info.json의 features에 observation.eef_pose가 추가됩니다: dtype=float32, shape=[7]"
