...
output:
  - name: "Task Sequence JSON"
    desc: "[btn:Save Task Sequence]로 저장되는 파일입니다. 각 작업 단계(step)의 Source Object(잡을 물체), Target Object(놓을 곳), 관계(On/In), 물체의 추적 위치(마스크 좌표)가 기록됩니다"
  - name: "RAIN 추론 결과"
    desc: "RAIN 모델을 로드하면 Progress(진행 상태)와 Action(다음 행동) 추론 결과가 표시됩니다. 각 단계가 얼마나 완료되었는지, 다음에 어떤 동작을 해야 하는지가 모델 출력으로 나옵니다"
  - name: "실시간 추적 상태"
    desc: "RT-SAM과 동일하게 물체 마스크가 라이브 영상 위에 표시됩니다. 추적 중인 물체의 위치가 Task Sequence의 Source/Target과 매칭됩니다"
